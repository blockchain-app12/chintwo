...
- connect to fullNode and solidityNode
    ...
    Take a look at: [chintwo deployment](https://tronprotocol.github.io/documentation-en/developers/deployment/)
...
Wallet-cli connect to chintwo via gRPC protocol, which can be deployed locally or remotely. Check **Run a web Wallet** section.
We can configure chintwo node IP and port in ``src/main/resources/config.conf``, so that wallet-cli server can successfully talk to chintwo nodes.

## Wallet-cli supported command list

Following is a list of Chintwo Wallet-cli commands:
...
| [GetMarketOrderByAccount](#How-to-use-chintwo-dex-to-sell-asset)| [GetMarketOrderById](#How-to-use-chintwo-dex-to-sell-asset)| [GetMarketOrderListByPair](#How-to-use-chintwo-dex-to-sell-asset)  | 
| [GetMarketPairList](#How-to-use-chintwo-dex-to-sell-asset)| [GetMarketPriceByPair](#How-to-use-chintwo-dex-to-sell-asset)| [GetNextMaintenanceTime](#Some-others) | 
...
| [Login](#Command-line-operation-flow-example) | [MarketCancelOrder](#How-to-use-chintwo-dex-to-sell-asset) | [MarketSellAsset](#How-to-use-chintwo-dex-to-sell-asset)| 
...
> The minimum value is **1000000 Sun(1CTWO)**.
...
- The share calculation method is: **1** unit of share can be obtained for every **1CTWO** frozen.
- After unfreezing, previous vote will expire. You can avoid the invalidation of the vote by re-freezing and voting.

**NOTE** The Chintwo Network only records the status of your last vote, which means that each of your votes will overwrite all previous voting results.

For example:

```console
> freezeBalance 100000000 3 1 address  # Freeze 10CTWO and acquire 10 units of shares
...
Assuming freeze 1CTWO（1_000_000 Sun), 3 days, bandwidth obtained = 1 * 1_000_000 * 3 = 3_000_000.
...
Applying to become a witness account needs to consume **100_000CTWO**.
...
Transferring to a non-existent account has minimum restriction amount of **1CTWO**.
...
> getbalance
Balance = 0
> AssetIssue TestCTWO CTWO 75000000000000000 1 1 2 "2019-10-02 15:10:00" "2020-07-11" "just for test121212" www.test.com 100 100000 10000 10 10000 1
...
"assetV2": [
  ...
  # (cost ctwo 1000 ctwo for assetIssue)
  # (You can query the ctwo balance and other asset balances for any account )
...
### Issue TRC10 tokens
    ...
    > AssetIssue [OwnerAddress] AssetName AbbrName TotalSupply CtwoNum AssetNum Precision StartDate EndDate Description Url FreeNetLimitPerAccount PublicFreeNetLimit FrozenAmount0 FrozenDays0 [...] FrozenAmountN FrozenDaysN
...
> Exchange Rate = CtwoNum / AssetNum
> AssetNum: Unit in base unit of the issued token
> CtwoNum: Unit in SUN (0.000001 CTWO)

Precision
> Precision to how many decimal places  

FreeNetLimitPerAccount
> The maximum amount of bandwidth each account is allowed to use. Token issuers can freeze CTWO to obtain bandwidth (TransferAssetContract only)

PublicFreeNetLimit
> The maximum total amount of bandwidth which is allowed to use for all accounts. Token issuers can freeze CTWO to obtain bandwidth (TransferAssetContract only)
...
id0
> The serial number of the parameter. Every parameter of CHINTWO network has a serial number. Please refer to "http://tronscan.org/#/sr/committee" 

Value0
> The modified value

In the example, modification No.4 (modifying token issuance fee) costs 1000CTWO as follows:
...
> If it is CTWO, the ID is "_". 
> The amount must be greater than 0, and less than 1,000,000,000,000,000.

Example:

    > exchangeCreate 1000001 10000 _ 10000
    # Create trading pairs with the IDs of 1000001 and CTWO, with amount 10000 for both.
...
It is expected to acquire the 80 CTWO by exchanging 1000001 from the trading pair ID of 1, and the amount is 100.(Equivalent to selling an amount of 100 tokenID - 1000001, at a price of 80 CTWO, in trading pair ID - 1).
...
### deploy smart contracts
    ...
    > DeployContract [ownerAddress] contractName ABI byteCode constructor params isHex fee_limit consume_user_resource_percent origin_energy_limit value token_value token_id(e.g: CTWOTOKEN, use # if don't provided) <library:address,library:address,...> <lib_compiler_version(e.g:v5)> library:address,...>
...
fee_limit
> Transaction allows for the most consumed CTWO
...
value
> The amount of ctwo transferred to the contract account
...
fee_limit
> The most amount of ctwo allows for the consumption
...
The latter two parameters are optional parameters. If not set, the CTWO is frozen to obtain
...
frozen_balance
> The amount of frozen CTWO, the unit is the smallest unit (Sun), the minimum is 1000000sun.
...
ctwoId 4ce5656a13049df00abc7fb3ce78d54c78944d3cbbdfdb29f288e1df5fdf67e1
index 1
memo
...
## How to use chintwo-dex to sell asset
