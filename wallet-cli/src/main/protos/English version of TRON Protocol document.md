...
## The protocol of CHINTWO is defined by Google Protobuf and contains a range of layers, from account, block to transfer.
...
```
     ...
     `asset`: other assets expect CTWO in this account – e.g. _{<“WishToken”,66666>,<”Dogie”,233>}_.
...
```    
     ...
     `amount`: the amount of CTWO – e.g. _12534_.  
...
```
     ...
     `ctwo_num`: the number of CHINTWOIX – e.g._232241_.  
...
+	Inventory is mainly used to inform peer nodes the list of items.  
    ...
    `type`: what type this `Inventory` is. – e.g. _0_ stands for `CTWO`.  
    `ids`: ID of things in this `Inventory`.

    Two `Inventory` types: `CTWO` and `BLOCK`.  
    `CTWO`: transaction.  
    `BLOCK`: block.
    ...
    `type`: type of items – e.g. _1_ stands for `CTWO`.  
    ...
    `Items` have four types: `ERR`, `CTWO`, `BLOCK` and `BLOCKHEADER`.  
    `ERR`: error.  
    `CTWO`: transaction.  
    `BLOCK`: block.  
